...
CREATE TABLE #НАЗНАЧЕНИЯ (
    ...
    [МСП-РаспорядительныйДокумент-РазмерМСП]                    VARCHAR(30),        --Элемент;  Обязательный;       Вещественное число с дробной частью в 2 знака   Актуальный на дату передачи сведений размер меры социальной поддержки
...
SELECT DISTINCT
    ...
    CONVERT(DATE, ISNULL(petition.A_DONEDATE, A_SERVDATE))  AS [МСП-РаспорядительныйДокумент-Реквизиты-Дата],
    servServ.A_NUMPB                    AS [МСП-РаспорядительныйДокумент-Реквизиты-Номер],
    CONVERT(VARCHAR, ROUND(payAmount.A_AMOUNT, 2))        AS [МСП-РаспорядительныйДокумент-РазмерМСП],
    npa.CODE_FROM_PFR                   AS [МСП-НПА],
...
SELECT DISTINCT
    ...
    CASE
        ...
        WHEN typeDoc.A_ID = 2596 THEN 'Суд г. Кирова'
        WHEN typeDoc.A_ID = 2164 THEN 'Военный комиссариат Кировской области'
...
----Перечень документов, подтверждающих право.
    INNER JOIN DOCUMENTS_CONFIRMING_RIGHT_FOR_PFR classifyDoc
        ON classifyDoc.CODE_FROM_PFR = #НАЗНАЧЕНИЯ.[МСП-КодМСП] 
----Заявления.
    INNER JOIN WM_PETITION petition 
        ON petition.OUID = servServ.A_REQUEST
----Класс связки Обращения-Документы
...
----Действующие документы.
    INNER JOIN WM_ACTDOCUMENTS actDocuments 
        ON actDocuments.OUID = petition_document.TOID
            ...
            AND (classifyDoc.DOCUMENT_HOLDER = 0
                OR classifyDoc.DOCUMENT_HOLDER = 1 AND actDocuments.PERSONOUID = servServ.A_PERSONOUID
                OR classifyDoc.DOCUMENT_HOLDER = 2 AND actDocuments.PERSONOUID = servServ.A_CHILD 
            )
----Вид документа.
    INNER JOIN PPR_DOC typeDoc
        ON typeDoc.A_ID = actDocuments.DOCUMENTSTYPE 
            AND typeDoc.A_ID = classifyDoc.CONFIRMING_RIGHT_DOC
...
--Выборка.
INSERT INTO #АДРЕСА ([Идентификатор], [УТ2:GUID], [УТ2:Индекс], [УТ2:Регион-Т2:Название], [УТ2:Регион-УТ2:Сокращение], [УТ2:Район-Т2:Название], [УТ2:Район-УТ2:Сокращение],
...
SELECT	
    ...
    personalCardChild.[УТ2:Пол]                                         AS [МСП-ГИД-Ребенок-УТ2:Пол],     
...
SELECT 
    NEWID()     AS [GUID],
    CONVERT(VARCHAR, CONVERT(DATE, @currentDateTime)) + 'T' + SUBSTRING(CONVERT(VARCHAR, CONVERT(TIME, @currentDateTime)), 0, 11)  AS [АФ5:ДатаВремя]
