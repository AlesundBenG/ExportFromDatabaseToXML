...
namespace ExportFromDatabaseToXML.Classes
{
    ...
    public class GeneratorSQLCommand
    {
        ...
        /// <summary>
        /// Получить одиночные поля для вставки.
        /// </summary>
        /// <param name="command">SQL запрос.</param>
        /// <returns>Список полей.</returns>
        public List<Token> getSingleParametrsFromCommand(string command) {
            List<Token> singleValue = new List<Token>();
            int currentPosition = 0;
            while (currentPosition < command.Length) {
                int startPosition = command.IndexOf('#', currentPosition);
                if (startPosition == -1) {
                    break;
                }
                int endPosition = goWhileNotMeetThoseSymbols(command, startPosition + 1, new char[3] { ' ', '\t', '#'});
                if (endPosition == -1) {
                    break;
                }
